...
"""
@Author  :   王超逸
@File    :   threadpool_wcy.py
@Time    :   2020/9/15 13:07
@Desc    :   复制过来并添加了ResultWrapper和相应的实现
原文档 https://chrisarndt.de/projects/threadpool/
因为是MIT授权，我觉得我可以把它复制一份
"""
...
class ResultWrapper(object):
    ...
    def status(self, value):
        ...
        if self._status_change_callback is not None:
            self._status_change_callback(self, value)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                result_warp = request.result
                result_warp.status = ResultWrapper.RUNNING
                result_warp.thread = self
                if self._dismissed.isSet():
...
class WorkRequest:
    ...
    def __init__(self, callable_, args=None, kwds=None, requestID=None,
        ...
        self.args = args or []
        self.kwds = kwds or {}
        self.result = ResultWrapper(requestID)
        self.result._status_change_callback = status_change_callback
...
class ThreadPool:
    ...
    def consume(self):
        """
        与poll方法相似，但是在没有任务的时候会阻塞住而不是抛出异常
        :return: None
        """
        while True:
            # 所有任务处理完毕，触发task_done_event
            # 好像不用判断队列，但是判断了显然也没啥问题
            if len(self.workRequests) == 0 and self._requests_queue.empty:
                self.task_done_event.set()
            # get back next results
            request, result = self._results_queue.get(block=True)
            # has an exception occured?
            if request.exception and request.exc_callback:
                request.exc_callback(request, result)
            # hand results to callback, if any
